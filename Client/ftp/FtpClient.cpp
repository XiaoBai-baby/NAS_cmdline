...
FtpClient::FtpClient(OS_TcpSocket sock, int bufsize) : m_SendSock(sock), m_buffer(bufsize), client_system(0)
{
	// 判断是否为 Windows系统;
#ifdef _WIN32
	client_system = 1;
...
int FtpClient::getCheck(string homeDir, char* argv[128], int out_argc)
{
	// 用于检查 get 命令;
	string path = "";
...
void FtpClient::in_get(string cmdline, char* argv[128], int argc)
{
	...
	for (int i = 1; i < argc; i++)
	{
		...
		if (length >= 0 || test == ".")
		{
			printf("reason: Don't use '..' symbol downloads the file, it is not allowed . \n\n");
			return;
		}
	}
...
void FtpClient::Start(const char* username,const char* password)
{
...
bool FtpClient::checkCmdine(char* cmdline, int cmdline_length)
{
	// 用于检查是否有错误命令;

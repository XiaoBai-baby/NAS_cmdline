...
public:
	...
	FileHandler(OS_TcpSocket RecvSock, string homeDir, bool is_win, bool isClient = false, int size = 1024 * 1024 * 300);
	...
	// 该模板函数 用于类的初始化;
	void operator()(OS_TcpSocket RecvSock, string homeDir, bool is_win, bool isClient = false, int size = 1024 * 1024 * 300);
...
public:
	...
	// 在Linux下, 遍历全部文件, 辅助 Linux_SendFile使用;

	...
	: m_RecvSock(recv_sock), m_buffer(bufsize), m_bufsize(bufsize)
{
	...
	m_file(m_RecvSock, m_homeDir, isWin);
...
string FtpService::on_cd(Json::Value& jresult)
{
	...
	if (argc > 1)
	{
		...
		if (result.length() <= 0)
		{
			...
			{
			#ifdef _WIN32
				string result2 = Utf8ToGbk(argv[1]);
				
				// memcpy 和 memcpy_s 属于内存拷贝，所以在拷贝过程中即便遇到 \0，也不会结束的;
				// memcpy_s((char*)cmdline.c_str(), strlen(cmdline.c_str()), result2.c_str(), result2.size());				// 内存错误;
				strcpy_s((char*)cmdline.c_str(), cmdline.size(), result2.c_str());											// 需要覆盖掉 cmdline整个内存;
			#else
				char cmdline2[128] = {0};
				GbkToUtf8(argv[1], strlen(argv[1]), cmdline2, 128);
				strcpy((char*)cmdline.c_str(), cmdline2);
			#endif
			}
			else
			{
			...
			// 在Linux下, 将报错 (即真实的内存为 m_path + cmdline + "\000\322/");
			// m_path += cmdline;
...
string FtpService::HandleCd(string& directory, int& find_pos, int& count, int& position, int (*argv)[128])				// 注意数组指针的优先级：()>[]> *
{
	...
	do
	{
		...
		argv[0][count] = find_pos - 2;					// 行指针也是一维数组, 如: argv[0][...]
		find_pos -= 3;
		count++;
	...
	if (count > 0)
	{
		...
		{
			// 取得最后一个 "/../"之后的字符串;
			long long substr_len = long (*argv[0] + 3);
